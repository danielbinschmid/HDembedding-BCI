...
warnings.filterwarnings(
    "ignore",
    message="Legacy autograd function with non-static forward method is deprecated and will be removed in 1.3. Please use new-style autograd function with static forward method",
)

warnings.filterwarnings(
    "ignore",
    message="Pass C=0.05, kernel=linear, degree=3, gamma=scale, coef0=0, shrinking=True, probability=False, tol=1e-05, cache_size=200, class_weight=None, verbose=False, max_iter=-1, decision_function_shape=ovr, break_ties=False, random_state=1 as keyword args. From version 0.25 passing these as positional arguments will result in an error",
)
...
code = [
    "thermometer"
]  # {'random_proj_bp_sep','learn_HD_proj_SGD','learn_HD_proj_ls','learn_HD_proj_unsup',
...
if DATASET == "IV2a":
    ...
    # HD_dim_vec2 = [10879]
    HD_dim_vec = {
        "learn_HD_proj_ls": HD_dim_vec1,
        "learn_HD_proj_SGD": HD_dim_vec1,
        "random_proj_bp": HD_dim_vec2,
        "random_proj_gm": HD_dim_vec2,
        "random_proj_bp_sep": HD_dim_vec2,
    }
    ...
    HD_dim_vec2 = [1000, 2000, 5000]
    # HD dimensions: depending on embedding different vector is taken
    HD_dim_vec1 = [10000]  # 500, 1000, 2000, 5000, 10000
    HD_dim_vec = {
        "learn_HD_proj_ls": HD_dim_vec1,
        "learn_HD_proj_SGD": HD_dim_vec1,
        "random_proj_bp": HD_dim_vec2,
    ...
    N_feat_per_band = {"single": 130, "spat": 10, "spat_bind": 10}
    # d=[5,9,37,75,369,737]
    # d = [13]
    d = [369]
                    ...
                    for model.t_vec in time_windows:
                        for model.f_band in f_band:
                            for model.k in k:
                                ...
                                if (
                                    model.code == "random_proj_gm"
                                    or model.code == "learn_HD_proj_ls"
                                    or model.code == "learn_HD_proj_SGD"
                                ):

                                    model.N_feat_per_band = N_feat_per_band[
                                        model.encoding
                                    ]
                                ...
                                # matrix projections with sparse entries
                                    ...
                                    model.code == "random_proj_bp"
                                    or model.code == "random_proj_bp_sep"
                                ):
                                    model.N_feat_per_band = N_feat_per_band[
                                        model.encoding
                                    ]
