...
class proj_trainer_end_end:
    def __init__(
        self, feat_dim, n_bands, HD_dim, n_classes, N_epoch, device, log_interval=50
    ):
        ...
        self._target_mem = (
            torch.ShortTensor(n_classes, self.HD_dim).bernoulli_().to(self.device)
        )
    ...
    def test(self, data, target, epoch, do_print=False):
        ...
        if do_print:
            print(
                "Epoch: {}, \t  Training set: Average loss: {:.4f}, Accuracy: {:.5f}".format(
    ...
    def get_params(self):
        ...
        return (
            self._target_mem,
            proj_mat,
            self.model.enc_vec.transpose(1, 0),
        )  # self.model.enc_vec#
