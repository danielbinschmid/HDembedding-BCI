...
class hd_classifier(lsh):
    def __init__(
        ...
        super().__init__(
            feat_dim, HD_dim, d, code, sparsity, cuda_device=cuda_device
        )  # init lsh for encoding real valued feature vector to binary HD vector
        ...
        self.ClassItem = (
            torch.ShortTensor(self.NO_classes, self.HD_dim).zero_().to(self.device)
        )
        ...
        if self.encoding == "single":  # need only one vector in single decoding
            ...
            self.enc_vec = (
                torch.ShortTensor(self.HD_dim, self.spat_dim)
                .bernoulli_()
                .to(self.device)
            )  # bernoulli
            ...
            raise ValueError(
                "Invalid encoding value. Got "
                + self.encoding
                + " and expected of spat or single"
            )
    ...
    def average_fit(self, samples, labels, n_iter=0):
        ...
        for i in range(NO_samples):
            ...
            S, cnt = self.transform(
                samples[i], clipping=False
            )  # get transformed HD_vector(not thresholded yet)
            ClassItem[labels[i] - 1].add_(S)
            class_count[labels[i] - 1] += cnt

        # Thresholding of Assotiative Memory
        self.ClassItem = self.thresh_item(
            ClassItem, self.NO_classes, class_count, self.HD_dim
        )  # Thresholding of
    ...
    def fit_learn_proj_sgd(self, samples, labels, n_iter=0):
        ...
        end_end_model = proj_trainer_end_end(
            N_feat, N_bands, self.HD_dim, self.NO_classes, N_epoch, self.device
        )
    ...
    def predict(self, samples):
        ...
        for smpl_idx in range(0, NO_samples):
            ...
            for test_class in range(0, self.NO_classes):
                HD_score[smpl_idx, test_class] = 1 - self.ham_dist(
                    S, self.ClassItem[test_class]
                )  # get value back in cpu
    ...
    def single_predict(self, S):
        ...
        for test_class in range(0, self.NO_classes):
            HD_score[test_class] = 1 - self.ham_dist(
                S, self.ClassItem[test_class]
            )  # get value back in cpu
            # Estimated Class is the one with maximum HD_score (+1 b/c classes [1,2,3,4])
    ...
    def spat_transform(self, sample, clipping=True):
        ...
        for item in range(self.spat_dim):
            hd_vec.add_(
                self.xor(self.enc_vec[:, item], self.encode(sample[item], band=item))
            )
        if clipping:
            # do thresholding
        ...
        for i in range(NO_item):
            if item_count[i] % 2 == 0:  # even number of counts
                if add_dim == 0:  # add a dim- dimensional tensor
                    Item[i].add_(
                        torch.ShortTensor(dim).bernoulli_().to(self.device)
                    )  # add random vector
                else:  # add a dim x add_dim - dimensional tensor
                    Item[i].add_(
                        torch.ShortTensor(add_dim, dim).bernoulli_().to(self.device)
                    )  # add random vector
    ...
    def ham_dist(self, vec_a, vec_b):
        ...
        rel_dist = 1 - torch.sum(vec_a == vec_b).cpu().numpy() / float(
            torch.numel(vec_a)
        )
