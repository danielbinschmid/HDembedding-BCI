...
class Net(nn.Module):
    ...
    def __init__(self, feat_dim, n_bands, HD_dim, device):
        ...
        self.enc_vec = (
            torch.ShortTensor(n_bands, HD_dim).bernoulli_().to(self.device)
        )  # random 0,1 vector
...
class Net1(nn.Module):
    ...
    def __init__(self, feat_dim, n_bands, HD_dim, device):
        ...
        self.fc1 = nn.ModuleList(
            [nn.Linear(feat_dim, HD_dim, bias=False) for i in range(n_bands)]
        )
    ...
    def get_weight(self):
        ...
        out_tensor = torch.Tensor(self.n_bands, self.HD_dim, self.feat_dim).to(
            self.device
        )
...
class Net2(nn.Module):
    ...
    def __init__(self, feat_dim, n_bands, HD_dim, device):
        ...
        if not ((HD_dim % n_bands) == 0):
            raise ValueError(
                "HD dim = {:d} not multiple of n_bands = {:}".format(HD_dim, n_bands)
            )
        ...
        self.enc_vec = (
            torch.ShortTensor(HD_dim).zero_().to(self.device)
        )  # random 0,1 vector
        # layers

        self.fc1 = nn.ModuleList(
            [nn.Linear(feat_dim, self.subHD_dim, bias=False) for i in range(n_bands)]
        )
    ...
    def forward(self, x):
        ...
        for band_idx in range(self.n_bands):
            ...
            x1[
                :, band_idx * self.subHD_dim : (band_idx + 1) * self.subHD_dim
            ] = self.fc1[band_idx](x[:, band_idx])

        return x1

    def get_weight(self):

        out_tensor = torch.Tensor(self.n_bands, self.subHD_dim, self.feat_dim).to(
            self.device
        )
