...
class lsh:
    def __init__(self, NO_feat, HD_dim, d, code, sparsity=0.9, cuda_device="cuda:0"):
        ...
        if code == "thermometer":
            self.HD_dim = (
                d * NO_feat
            )  # HD dimension depends on number of quantization bits
            self.d = d  # number bits for quantization
            self.dict = (
                torch.from_numpy(np.tril(np.ones(self.d, dtype=int)))
                .short()
                .to(self.device)
            )
            ...
            self.HD_dim = HD_dim
            self.proj_mat = (
                torch.FloatTensor(self.HD_dim, self.NO_feat).normal_().to(self.device)
            )
            ...
            proj_mat_cpu = (
                SparseRandomProjection(HD_dim, density=1 - sparsity)
                .fit(np.zeros((1, NO_feat)))
                .components_.todense()
            )
            ...
            code == "learn_HD_proj_ls"
            or code == "learn_HD_proj_SGD"
            or code == "learn_HD_proj_unsup"
        ):
    ...
    def get_greyish_code(self, d):
        ...
        for ii in range(self.q):
            for jj in range(ii + 1, d):
                ...
                row += 1
        self.dict = (
            torch.from_numpy(self.dict[: self.q].astype(int)).short().to(self.device)
        )
    ...
    def encode_quant(self, sample, band=0):
        ...
        sample = (
            ((sample - self.mean) / (self.sigma)) * self.q + (self.q - 1) / 2
        ).astype(
            int
        )  # add q term for quantization
    ...
    def load_model_data(self, subject, fold, path):
        ...
        with np.load(load_path) as data:
            ...
            if not (NO_feat == self.NO_feat and HD_dim == self.HD_dim):
                raise ValueError(
                    "Number of features and HD dim don't match with loaded lsh mat"
                )
