...
class KMeans:
    ...
    def fit(self, X):
        ...
        if self.verbose:
            print(
                "Start K-menas calculation for {:d} centroids".format(self.n_clusters)
            )
        ...
        for i_init in range(self.n_init):
            ...
            while (not np.array_equal(new_label, label)) and (iterr < self.max_iter):
                ...
                new_centroids = (
                    torch.ShortTensor(self.n_clusters, HD_dim).zero_().to(self.device)
                )
                ...
                for samp_idx in range(n_samples):
                    ...
                    dist = self.get_multi_HD_dist(
                        X[samp_idx], cur_centroids, self.n_clusters
                    )
            ...
            ):  # first round or new best partition
            ...
            if self.verbose:
                print(
                    "Iteration {}, Cost: {:.2f}, Best cost:  {:.2f}: ".format(
                        i_init, cost, self._best_cost
                    )
                )
    ...
    def labels_inertia(self, X):
        ...
        for samp_idx in range(n_samples):
            dist[samp_idx] = self.get_multi_HD_dist(
                X[samp_idx], self.cluster_centers_, n_item=self.n_clusters
            )
    ...
    def thresh_item(self, Item, item_count):
        ...
        for i in range(NO_item):
            if item_count[i] % 2 == 0:  # even number of counts
                Item[i].add_(
                    torch.ShortTensor(HD_dim).bernoulli_().to(self.device)
                )  # add random vector
                item_count[i] += 1
