    ...
    for band_idx in range(n_bands):
        if ftype == "butter":
            ...
            filter_bank[band_idx] = signal.firwin(
                order, f_band_nom[band_idx], pass_zero=False
            )
    return filter_bank
...
def butter_fir_filter(signal_in, filter_coeff):
    ...
    if filter_coeff.ndim == 2:  # butter worth
        ...
        for channel in range(0, NO_channels):
            sig_filt[channel] = signal.convolve(
                signal_in[channel, :], filter_coeff, mode="same"
            )  # signal has same size as signal_in (centered)
