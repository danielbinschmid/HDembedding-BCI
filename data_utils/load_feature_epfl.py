...
def load_feature_EPFL(
    ...
    NO_subjects = 5
    NO_riem = int(
        NO_channels * (NO_channels + 1) / 2
    )  # Total number of Riemannian feature per band and timewindow
    ...
    if load_features:
        ...
        train_feat, train_label, test_feat, test_label = generate_Riemann_feat(
            data_path, subject, twind_sel, band_sel, feat_type, split
        )
    ...
    if encoding == "single":
        ...
        test_feat = np.reshape(
            np.transpose(test_feat, (0, 2, 1, 3)), (N_test_trial, 1, N_bands, -1)
        )
...
def generate_Riemann_feat(data_path, subject, twind_sel, band_sel, riem_settings, fold, downsample_factor=2):

    ################################### SETTINGS ######################################################
    fs = 512.0 / downsample_factor  # sampling frequency
    NO_channels = 16  # number of EEG channels
    NO_riem = int(
        NO_channels * (NO_channels + 1) / 2
    ...
    forder = 1  # 4
    filter_bank = load_filterbank(
        bw, fs, order=forder, max_freq=30, ftype=ftype
    ...
    train_data, train_label, test_data, test_label = get_data_epfl(
        data_path, subject, fold, downsample_factor=downsample_factor
    )
