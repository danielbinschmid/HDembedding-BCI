...
def load_feature_IV2a(
    ...
    if load_features:
        ...
        if feat_type == "CSP":
            train_feat = train_feat[
                np.ix_(np.arange(N_tr_trial), twind_sel, band_sel)
            ].reshape(N_tr_trial, N_twind, N_bands, 24)
            test_feat = test_feat[
                np.ix_(np.arange(N_ev_trial), twind_sel, band_sel)
            ].reshape(N_ev_trial, N_twind, N_bands, 24)
            N_feat = 24
        else:
        ...
        if feat_type == "CSP":
            train_feat, train_label, test_feat, test_label = generate_CSP_feat(
                data_path, subject, twind_sel, band_sel, feat_type
            )
            N_feat = 24
        elif feat_type == "Riemann":
            train_feat, train_label, test_feat, test_label = generate_Riemann_feat(
                data_path, subject, twind_sel, band_sel, feat_type
            )
            # N_feat = 253
        else:
    ...
    if encoding == "single":
        ...
        train_feat = np.reshape(
            np.transpose(train_feat, (0, 2, 1, 3)), (N_tr_trial, 1, -1)
        )
        test_feat = np.reshape(
            np.transpose(test_feat, (0, 2, 1, 3)), (N_ev_trial, 1, -1)
        )
        ...
        train_feat = np.reshape(
            np.transpose(train_feat, (0, 2, 1, 3)), (N_tr_trial, 1, N_bands, -1)
        )
        test_feat = np.reshape(
            np.transpose(test_feat, (0, 2, 1, 3)), (N_ev_trial, 1, N_bands, -1)
        )
        # reshape features for svm
        svm_train_feat = np.reshape(train_feat, (N_tr_trial, -1))
        svm_test_feat = np.reshape(test_feat, (N_ev_trial, -1))
...
def load_XVAL_feature_IV2a(
    ...
    feat, svm_feat, label, _, _, _ = load_feature_IV2a(
        subject, twind_sel, band_sel, feat_type, encoding, nfisher, split, n_splits
    )
...
def transform_covmat(train_feat, test_feat):
    ...
    for trial in range(N_tr_trial):
        for twind in range(N_twind):
            for band in range(N_bands):
                out_tr_feat[trial, twind, band] = half_vectorization(
                    train_feat[trial, twind, band]
                )
    ...
    for trial in range(N_ev_trial):
        for twind in range(N_twind):
            for band in range(N_bands):
                out_test_feat[trial, twind, band] = half_vectorization(
                    test_feat[trial, twind, band]
                )
...
def generate_Riemann_feat(data_path, subject, twind, freq_band, riem_settings):
    ...
    riemann = riemannian_multiscale(
        filter_bank, time_windows, riem_opt=riem_settings, rho=rho, vectorized=False
    )
    t1 = time.time()
