...
from svm_multires import svm_multires, svm_nonlinear
...
class Hd_model:
    def __init__(
        ...
        data_path,
        crossval=False,
        classifier="assotiative",
        cuda_device="cuda:0",
        lda_svm_precision=64,
        save_features=False,
        load_features=False,
    ):
        ...
        # svm settings
        self.svm_kernel = "rbf"  #'sigmoid'#'linear' # 'sigmoid', 'rbf',
    ...
    def test_hd(self, fold=0):
        # load features
        train_feat, svm_train_feat, train_label, eval_feat, svm_eval_feat, eval_label = self.load_feature(
            self.data_path,
            self.subject,
            self.t_vec,
            self.f_band,
        ...
        clf_SVM = svm_multires(
            ...
            random_state=1,
            tol=0.00001,
            precision=self.lda_svm_precision,
        )

        clf_SVM = svm_nonlinear(
            C=self.svm_c, multi_class="ovr", random_state=1, tol=0.00001
        )

        clf_LDA = lda_multires(
            solver="lsqr", shrinkage="auto", precision=self.lda_svm_precision
        )

        # init HD classifier
        hd_b = self.hd_classifier(
            self.N_feat_per_band,
            self.N_bands,
            self.HD_dim,
            self.d,
            self.encoding,
    ...
    def run(self):
        ...
        for self.subject in range(1, self.NO_subjects + 1):
            ...
            print(
                "Subject{:}; {:0.4f};\t{:0.4f};\t{:0.4f};\t{:0.4f}".format(
                    self.subject, mean_suc[0], mean_suc[1], mean_suc[2], mean_suc[3]
                )
            )
        ...
        print(
            "AVG:;\t {:0.4f}; \t {:0.4f}; \t {:0.4f};\t{:0.4f}".format(
                avg_succ[0], avg_succ[1], avg_succ[2], avg_succ[3]
            )
        )
        ...
        np.savez(
            self.save_path,
            success=success,
            N_feat_per_band=self.N_feat_per_band,
            HD_dim=self.HD_dim,
            d=self.d,
            code=self.code,
